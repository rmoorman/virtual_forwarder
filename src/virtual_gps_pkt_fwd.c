...
//Ruud: to test the bin_to_b64
// source: http://stackoverflow.com/questions/6357031/how-do-you-convert-buffer-byte-array-to-hex-string-in-c
// author: http://stackoverflow.com/users/168465/kriss
...
{ int prt_size = 3*insz;
  char prt_str[prt_size];
  tohex(in,insz,prt_str,prt_size);
  printf("%s\n",prt_str);
}
...
//			}
			...
			buff_index += 9;
			//Ruud: aanpassing om ingaande data te bekijken:
            //printf("binary input  = "); printhex(p->payload, p->size);
			j = bin_to_b64(p->payload, p->size, (char *)(buff_up + buff_index), 341); /* 255 bytes = 340 chars in b64 + null char */
			if (j>=0) {
				//Ruud: aanpassing om uitgaande data te bekijken:
				//printf("b64 output  = "); printf((char *)(buff_up + buff_index));
				buff_index += j;
			} else {
